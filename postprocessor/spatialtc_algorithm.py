...
from ..functions.SOLWEIGpython.COMFA.radiationfunctionsCOMFA import (
    COMFA_RAD_SPATIAL_TC as COMFA_rad,
)
...
def load_grid(filepath, feedback):
    ...
    return (
        ...
        temp_grid.ReadAsArray().astype(float).shape[1],
    )
...
class ProcessingSpatialTCAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        #                                             optional=False))
        self.varType = (
            (self.tr("Physiological Equivalent Temperature (PET)"), "0"),
        ...
        # COMFA or COMFA-kid
        comfa = QgsProcessingParameterBoolean(
            self.COMFA,
            self.tr("COMFA-kid (Cheng and Brown, 2020)"),
            defaultValue=False,
        )
        comfa.setFlags(
            comfa.flags() | QgsProcessingParameterDefinition.Flag.FlagAdvanced
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if metdata.shape[1] == 24:
            ...
            raise QgsProcessingException(
                "Error: Wrong number of columns in meteorological data. Use the forcing file "
        ...
        if np.where(metdata[:, 2] == hoursTmrt).__len__() == 1:
            posMet = (
                np.where(metdata[:, 0] == yyyyTmrt)
                and np.where(metdata[:, 1] == doyTmrt)
                and np.where(metdata[:, 2] == hoursTmrt)
            )
        else:
            posMet = (
                np.where(metdata[:, 0] == yyyyTmrt)
                and np.where(metdata[:, 1] == doyTmrt)
                and np.where(metdata[:, 2] == hoursTmrt)
                and np.where(metdata[:, 3] == minuTmrt)
            )
        ...
        current_time = str(
            (
                pd.to_datetime(YYYY, format="%Y")
                + pd.to_timedelta(jday - 1, unit="d")
        ...
        feedback.setProgressText(
            ...
            + " latitude,  "
            + str(np.around(lon, decimals=2))
            + " longitude"
        )
        feedback.setProgressText(
            "Air temperature derived from meteorological data is: "
            + str(Ta)
            + " "
            + "\N{DEGREE SIGN}C"
        )
        ...
        if not (rows == rows2) & (cols == cols2):
            raise QgsProcessingException(
                "Error: Wind speed raster not same domain as Tmrt raster: All rasters must be of same extent and resolution"
            )

        if not (rows == rows3) & (cols == cols3):
            raise QgsProcessingException(
                "Error: Buildings raster not same domain as Tmrt raster: All rasters must be of same extent and resolution"
            )
        ...
        if tcType == 0:
            ...
            if comfa_kid:
                ...
                feedback.setProgressText(
                    "Calculating energy balance (COMFA) for all ground level pixels"
                )
            # COMFA parameters
            # Settings
