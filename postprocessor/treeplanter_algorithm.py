...
class ProcessingTreePlanterAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSource(
                ...
                self.tr("Planting area (Vector polygon)"),
        ...
        ttype = ((self.tr("Deciduous"), "0"), (self.tr("Conifer"), "1"))
        self.addParameter(
            QgsProcessingParameterNumber(
                self.START_HOUR,
                self.tr("From (hour)"),
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.OUTPUT_CDSM,
                self.tr("Save Canopy Digital Surface Model"),
                defaultValue=True,
            )
        )
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.OUTPUT_POINTFILE,
                self.tr("Save Vector point file with tree location(s)"),
                defaultValue=True,
            )
        )
        self.addParameter(
            QgsProcessingParameterBoolean(
                ...
                self.tr(
                    "Save Occurrence map (Only available when running with random or genetic algorithms and more than one tree)"
                ),
                defaultValue=False,
            )
        )
        ...
        includeOutside = QgsProcessingParameterBoolean(
            self.INCLUDE_OUTSIDE,
            self.tr(
                "Allow areas outside of Planting area to be included in calculation"
            ),
            defaultValue=True,
        )
        ...
        greedyAlgorithm = QgsProcessingParameterBoolean(
            self.GREEDY_ALGORITHM,
            self.tr("Use a greedy algorithm to position trees"),
            defaultValue=False,
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if outputOccurrence & (nTree > 1) & (not greedy):
            ...
            feedback.setProgressText(
                "Finding location for one tree. Occurrence raster will not be produced."
            )
        elif greedy:
            feedback.setProgressText(
                "Running with greedy algorithm. Occurrence raster will not be produced."
            )

        feedback.setProgressText(
            "Starting model at " + str(datetime.datetime.now()) + "..."
        )
        ...
        tree_input = Inputdata(
            r_range, sh_fl, tmrt_fl, infolder, inputPolygonlayer, feedback
        )
        ...
        if not outside_selected:
            ...
            for i in np.arange(tree_input.tmrt_ts.shape[2]):
                ...
                tree_input.shadow[:, :, i] = (
                    tree_input.shadow[:, :, i] * tree_input.selected_area
                )

        # Tmrt for shaded point
        tmrt_1d, azimuth, altitude, amaxvalue = tmrt_1d_fun(
            INPUT_MET,
            infolder,
            transVeg,
            tree_input.lon,
        ...
        cdsm_, tdsm_ = makevegdems.vegunitsgeneration(
            buildings_empty,
            cdsm_,
            tdsm_,
            treedata.ttype,
        ...
        for i in r_range:
            ...
            treesh_sum_tmrt[:, :] = (
                treesh_sum_tmrt + treesh_ts2[:, :, i_c] * tmrt_1d[i_c, 0]
            )
            i_c += 1
        ...
        treerasters = Treerasters(
            treesh_sum_tmrt, shadow_rg.shadow, treesh_ts1, cdsm_, treedata
        )
        ...
        if greedy:
            ...
            t_y, t_x, tmrt_max = GreedyAlgorithm.greedyplanter(
                cropped_rasters,
                treedata,
                treerasters,
                tmrt_1d,
            ...
            if starting_algorithm:
                sa = 0
                feedback.setProgressText(
                    "Running hill climbing algorithm with random algorithm for starting positions..."
                )
            else:
                sa = 1
                feedback.setProgressText(
            ...
            if nTree == 1:
                ...
                t_y, t_x, tmrt_max, t_y_all, t_x_all = (
                    TreePlanterHillClimber.treeoptinit(
                        treerasters,
                        cropped_rasters,
                        positions,
                        treedata,
                ...
                if outputOccurrence:
                    ...
                    for row in np.arange(t_y_all.shape[0]):
                        for col in np.arange(t_y_all.shape[1]):
                            occurrence_map[
                                t_y_all[row, col].astype(int),
                                t_x_all[row, col].astype(int),
                            ] += 1
        ...
        for i in range(0, t_y.shape[0]):
            cdsm_temp, tdsm_temp = makevegdems.vegunitsgeneration(
                bld_orig,
                cdsm_empty,
                tdsm_empty,
                treedata.ttype,
        ...
        minx, x_size, x_rotation, miny, y_rotation, y_size = (
            tree_input.dataSet.GetGeoTransform()
        )
        for i in range(0, t_y.shape[0]):
    ...
    def shortHelpString(self):
        return self.tr(
            "This is a model for optimization of tree arrangement to mitigate thermal "
