...
class ProcessingTARGETAnalyzerAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.SINGLE_DAY_BOOL,
                self.tr("Examine single night"),
                defaultValue=False,
                optional=True,
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        start = datetime.datetime.strptime(cfM["date1"], "%Y,%m,%d,%H").strftime(
            "%Y-%m-%d"
        )  # string
        end = datetime.datetime.strptime(cfM["date2"], "%Y,%m,%d,%H").strftime(
            "%Y-%m-%d"
        )  # string
        ...
        feedback.setProgressText(
            "Days indentified as modelled by TARGET: " + start + " to " + end
        )
        ...
        if singleNight:
            ...
            if startDate >= end or startDate < start:
                raise QgsProcessingException(
                    "Single date has to be within the modelled days OR the same as the last modelled day."
                )
            else:
                feedback.setProgressText(
                    "Single day and following night to examine: "
        ...
        for f in vlayer.getFeatures():
            ...
            datawhole = np.genfromtxt(
                targetIn + "/output/" + runName + "_" + gid + "_UMEP_TARGET.txt",
                skip_header=1,
            )
        ...
        if irreg:
            ...
            for (
                ...
                vlayer.getFeatures()
            ):  # Taking first polygon. Could probably be done nicer
        ...
        self.rasterize(
            polygonpath,
            str(self.plugin_dir + "/tempgrid.tif"),
            str(poly_field[0]),
            resx,
            crs,
            extent,
    ...
    def shortHelpString(self):
        return self.tr(
            "The <b>TARGET Analyzer</b> plugin can be used to make basic grid analysis of model results generated by the TARGET model.<br>"
            "\n"
            "--------------\n"
