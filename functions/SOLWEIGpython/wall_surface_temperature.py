...
def load_walls(
    ...
    # Initial wall temperature is set to air temperature
    ...
    # Calculate fractions
    # Non-sky fraction = (1 - svf at ground level) - 0.5 (ground surface seen
    # from wall)
    ...
    # Set thermal effusivity according to wall type (either from GUI or from
    # landcover)
    if landcover == 1:
        unique_landcover = np.unique(lcgrid)
        # Unique wall codes for unique wall types, e.g. brick, concrete, wood,
        # etc.
...
def surface_temperature_calc(
    ...
    # Calculate heat flux based on Ts in previous time step and Kin, Lin and
    # Ta from current time step
    ...
    # Surface temperature
    Ts_current = Ta + dT
    # Estimate surface temperature with Ts of current timestep, i.e. not
    # Ts_previous
...
def wall_surface_temperature(
    ...
    # Starting value is zero, i.e. the entire wall is shaded
    voxelTable["wallShade"] = 0
    # If sun is above horizon, get wall shadow height on wall from wallsh
    # calculated in shadowingfunction_wallheight_23/13 in
    # Solweig_2022a_calc_forprocessing.py
    if altitude > 0:
    ...
    # Ldown and Lup for wall pixels used when estimating the amount of
    # longwave received from surrounding surfaces (ground and reflected)
    ...
    if altitude > 0:
        ...
        # Only half of hemisphere visible from a wall, therefore the entire
        # seen area can be sunlit, compared to an open space where only half
        # can be sunlit
        F_sh = 2.0 * F_sh - 1.0  # (cylindric_wedge scaled 0-1)

        # Fraction of sunlit and shaded building surfaces seen by a building
        # wall based on its aspect and the azimuth of the sun
        ...
        # Temperature in shade and sun based on mean values for all voxels,
        # i.e. shade = mean of all shaded and sun = mean of all sunlit
        ts_shade = (
        ...
        # Calculation of longwave radiation received by the voxels based on
        # sunlit and shaded surroundings
        Lwallsun = (
            SBC
    ...
    Lsky = SBC * esky * ((Ta + 273.15) ** 4) * voxelTable["SVF_fix"].to_numpy()
