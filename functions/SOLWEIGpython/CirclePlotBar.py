...
def PolarBarPlot(
    ...
    for i in range(skyalt_c.__len__()):
        ...
        if skyalt_c[i] > 1:
            theta_patch = (
                ...
                * (360 / skyalt_c[skyalt == skyalt[i]][0])
            ) * deg2rad
            width_patch = (
                np.ones(skyalt_c[skyalt == skyalt[i]][0])
                * (360 * deg2rad)
            ...
            for j in patch_order_range:
                ...
                bars = ax.bar(
                    theta_patch[j] + (azistart[i] * deg2rad),
                    radii[j] - radii_sub,
                    width=width_patch[j],
                    bottom=0.0,
                    edgecolor="black",
            ...
            my_circle = Circle(
                (0, 0), 0.1, edgecolor="black", transform=ax.transData._b
            )
            if plot_type:
    ...
    if (plot_type == 0) & (solar_altitude > 0):
        sun_position = plt.scatter(
            (solar_azimuth * deg2rad),
            1.5 - (1.5 * (solar_altitude / 90)),
            s=(70 * (solar_altitude / 90)),
            color="gold",
        )
        sun_position_star1 = plt.scatter(
            (solar_azimuth * deg2rad),
            1.5 - (1.5 * (solar_altitude / 90)),
            s=(400 * (solar_altitude / 90)),
            color="gold",
            marker="1",
        )
        sun_position_star2 = plt.scatter(
            (solar_azimuth * deg2rad),
            1.5 - (1.5 * (solar_altitude / 90)),
            s=(400 * (solar_altitude / 90)),
            color="gold",
    ...
    if plot_type:
        ...
        sm = plt.cm.ScalarMappable(
            cmap=plt.cm.jet, norm=plt.Normalize(vmin=minrad, vmax=maxrad)
        )  # Watts per square meter Steradian
        sm._A = []
        ...
        legend_handles = [
            plt.Rectangle((0, 0), 1, 1, color=legend_colors[legend_label])
            for legend_label in legend_labels
        ]
        if solar_altitude > 0:
            legend_handles.extend(
                [(sun_position, sun_position_star1, sun_position_star2)]
            )
