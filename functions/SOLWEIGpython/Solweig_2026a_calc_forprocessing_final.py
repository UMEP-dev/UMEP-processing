...
def Solweig_2026a_calc(i, dsm, scale, rows, cols, svf, svfN, svfW, svfE, svfS, svfveg, svfNveg, svfEveg, svfSveg,
                       ...
                       Tgmap1E, Tgmap1S, Tgmap1W, Tgmap1N, CI, diffsh, shmat, vegshmat, vbshvegshmat, anisotropic_sky, asvf, patch_option,
                       voxelMaps, voxelTable, ws, wallScheme, timeStep, steradians, walls_scheme, dirwalls_scheme,
                       groundScheme, Tg,  Rn, Rn_past, G, Tm, cap_grid, diff_grid, a1_grid, a2_grid, a3_grid, 
                       shadow_past):
    """
    ...
    if altitude > 0: # # # # # # DAYTIME # # # # # #
        ...
        if groundScheme == 1:
            ...
            Tg, Rn, Rn_past, G = surfaceTemperature_calc(Kdown, Ldown, Rn, Rn_past, G, Tg, Tm, alb_grid, emis_grid, cap_grid, diff_grid, 
                                                                         lc_grid, a1_grid, a2_grid, a3_grid, timeStep, RH, shadow, shadow_past,)
            
            # # # # Lup, daytime # # # #
            Lup, gvfalb, gvfalbnosh, LupE, gvfalbE, gvfalbnoshE, LupS, gvfalbS, gvfalbnoshS, \
                LupW, gvfalbW, gvfalbnoshW, LupN, gvfalbN, gvfalbnoshN, \
                    gvfLsideW, gvfLsideS, gvfLsideE, gvfLsideN = outgoingLongwave_calc(Tg, Tgwall, Ta, Ldown, emis_grid, alb_grid, buildings, shadow, 
                                                                                        wallsun, walls, rows, cols, 1/scale)
            
        else :
            # using max sun alt instead of dfm
            Tgamp = TgK * altmax + Tstart # Fixed 2021
            Tgdiff = Tgamp * np.sin((((dectime - np.floor(dectime)) - SNUP / 24) / (TmaxLST / 24 - SNUP / 24)) * np.pi / 2) # 2015 a, based on max sun altitude

            Tgdiff = Tgdiff * CI_TgG  # new estimation

            # For Tg output in POIs
            TgTemp = Tgdiff * shadow + Ta
            _, timeadd, Tg = TsWaveDelay_2015a(TgTemp, firstdaytime, timeadd, timestepdec, Tg) #timeadd only here v2021a
    ...
    if groundScheme == 1:
        ...
        Least_, Lsouth_, Lwest_, Lnorth_ = Lside_veg_v2026(svfS, svfW, svfN, svfE, svfEveg, svfSveg, svfWveg, svfNveg,
                    svfEaveg, svfSaveg, svfWaveg, svfNaveg, azimuth, altitude, Ta, Tgwall, SBC, ewall, Ldown, esky, t, F_sh, CI, anisotropic_sky)
    else:        
        Least = np.zeros_like(Ldown)
        Lnorth = np.zeros_like(Ldown)
        Lwest = np.zeros_like(Ldown)
        Lsouth = np.zeros_like(Ldown)
        Least_, Lsouth_, Lwest_, Lnorth_ = Lside_veg_v2022a(svfS, svfW, svfN, svfE, svfEveg, svfSveg, svfWveg, svfNveg,
                    svfEaveg, svfSaveg, svfWaveg, svfNaveg, azimuth, altitude, Ta, Tgwall, SBC, ewall, Ldown, esky, t, F_sh, CI, LupE, LupS, LupW, LupN, anisotropic_sky)
    ...
    if (anisotropic_sky == 1):
        ...
        Ldown, Lside_, Lside_sky, Lside_veg, Lside_sh, Lside_sun, Lside_ref, Least_, Lwest_, Lnorth_, Lsouth_, \
    ...
    return Tmrt, Kdown, Kup, Ldown, Lup, Tg, ea, esky, I0, CI, shadow, firstdaytime, timestepdec, \
            ...
            Lsouth, Lwest, Lnorth, KsideI, radI, radD, \
            Lside, L_patches, CI_TgG, KsideD, dRad, Kside, steradians, voxelTable, \
            Rn, Rn_past, Tm, G