...
class URockAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.WIND_HEIGHT,
                self.tr(
                    'Output wind height(s) (m) - if several values, separated by ","'
                ),
                defaultValue="1.5",
                optional=False,
            )
        ...
        self.addParameter(
            QgsProcessingParameterFolderDestination(
                self.OUTPUT_DIRECTORY, self.tr("Directory to save the outputs")
            )
        )
        self.addParameter(
            QgsProcessingParameterString(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # Defines inputs
        javaEnvVar = self.parameterAsString(
            parameters, self.JAVA_PATH, context
        )
        ...
        profileFile = self.parameterAsString(
            parameters, self.INPUT_PROFILE_FILE, context
        )

        # Get building layer and then file directory
        inputBuildinglayer = self.parameterAsVectorLayer(
            parameters, self.BUILDING_TABLE_NAME, context
        )
        heightBuild = self.parameterAsString(
        ...
        if inputBuildinglayer:
            ...
            if not heightBuild:
                raise QgsProcessingException(
                    "A building height attribute should be defined"
                )
        else:
            build_file = None
            srid_build = None
        ...
        inputVegetationlayer = self.parameterAsVectorLayer(
            parameters, self.VEGETATION_TABLE_NAME, context
        )
        topHeightVeg = self.parameterAsString(
            parameters, self.VEGETATION_CROWN_TOP_HEIGHT, context
        )
        if inputVegetationlayer:
            ...
            if not topHeightVeg:
                raise QgsProcessingException(
                    "A vegetation crown top height attribute should be defined"
                )
        else:
            veg_file = None
            srid_veg = None
        ...
        if not veg_file and not build_file:
            raise QgsProcessingException(
                "Either building or vegetation file should be provided"
            )

        outputRaster = self.parameterAsRasterLayer(
            parameters, self.RASTER_OUTPUT, context
        )
        ...
        baseHeightVeg = self.parameterAsString(
            parameters, self.VEGETATION_CROWN_BASE_HEIGHT, context
        )
        attenuationVeg = self.parameterAsString(
            parameters, self.ATTENUATION_FIELD, context
        )
        # prefix = self.parameterAsString(parameters, self.PREFIX, context)

        # Defines outputs
        z_out = self.parameterAsString(
        ...
        if outputRaster:
            if (
                inputBuildinglayer.crs().postgisSrid()
                != outputRaster.crs().postgisSrid()
            ):
                feedback.pushWarning(
            ...
            raise QgsProcessingException(
                "You should either specify an output raster or a horizontal mesh size"
            )
        ...
        WriteMetadataURock.writeRunInfo(
            outputDirectory, build_file, veg_file, attenuationVeg
        )
        ...
        (
            ...
            javaEnvironmentPath=javaEnvVar,
        ...
        if loadOutput:
            for z_i in z_out:
                if saveVector:
                    loadedVector = QgsVectorLayer(
                        os.path.join(
                            outputDirectory,
                            "z{0}".format(str(z_i).replace(".", "_")),
                            outputFilename + OUTPUT_VECTOR_EXTENSION,
                        ),
    ...
    def shortHelpString(self):
        return self.tr(
            "The URock plugin can be used to calculate "
