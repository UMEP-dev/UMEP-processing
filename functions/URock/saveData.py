...
from .DataUtil import radToDeg, windDirectionFromXY, createIndex, prefix, safe
...
def saveBasicOutputs(
    ...
    cursor.execute(safe(""" SELECT ST_SRID({0}) AS srid FROM {1} LIMIT 1
...
def saveRasterFile(
    ...
    if outputRaster:
        ...
        resX = (
            outputRasterExtent.xMaximum() - outputRasterExtent.xMinimum()
        ) / outputRaster.width()
        resY = (
            outputRasterExtent.yMaximum() - outputRasterExtent.yMinimum()
        ) / outputRaster.height()
        ...
        output_ds = driver.CreateCopy(
            outputFilePathAndNameBaseRaster + OUTPUT_RASTER_EXTENSION, ds
        )
...
def interp_vec_to_rast(
    outputVectorFile,
    stacked_blocks,
    outputFilePathAndNameBaseRaster,
    extent,
    resX,
    resY,
    z_i,
    ...
    order_changed = processing.run(
        "native:orderbyexpression",
        {
            ...
            "ASCENDING": False,
            "NULLS_FIRST": False,
            "OUTPUT": os.path.join(tmp_dir, f"order_changed_{colname}"),
        },
    )["OUTPUT"]
    ...
    interp_out = processing.run(
        "qgis:tininterpolation",
        {
            ...
            "OUTPUT": os.path.join(tmp_dir, f"interp_out_{colname}.tif"),
        },
    )["OUTPUT"]
    ...
    if os.stat(stacked_blocks).st_size > 0:
        ...
        block_base = processing.run(
            "gdal:rasterize",
            {
                ...
                "INVERT": False,
                "EXTRA": "",
                "OUTPUT": os.path.join(tmp_dir, f"block_base_{colname}.tif"),
            },
        )["OUTPUT"]
        ...
        block_top = processing.run(
            "gdal:rasterize",
            {
                ...
                "INVERT": False,
                "EXTRA": "",
                "OUTPUT": os.path.join(tmp_dir, f"block_top_{colname}.tif"),
            },
        )["OUTPUT"]

        # Keep the values only when there is no building at this position
        output_file_path = processing.run(
