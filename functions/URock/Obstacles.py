...
def windRotation(
    ...
    columnNames = {}
    # Store the column names (except geometry field) of each table into a
    # dictionary
...
def createsBlocks(
    ...
    # Identify all possible values of height for buildings being in a more
    # than 1 building block
    cursor.execute(
        safe("""
       {0};
       """).format(
...
def identifyBlockAndCavityBase(cursor, stackedBlockTable, prefix=PREFIX_NAME):
    ...
    # Identify each block base height and ratio of area between the stacked
    # and its base block
    cursor.execute(
        safe("""
       {7};
    ...
    # Same as previous for stacked buildings being above a ground building
    # (not a stacked one...)
    cursor.execute(
        safe("""
       {9};
...
def initDownwindFacades(cursor, obstaclesTable, prefix=PREFIX_NAME):
    ...
    # Merge the ones that intersect each other and join stacked block
    # properties
    cursor.execute(
        safe("""
       DROP TABLE IF EXISTS {0};
