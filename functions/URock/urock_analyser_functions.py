...
def plotSectionalViews(
    ...
    if polygons_file and srid_polygons and idPolygons:
        if feedback:
            feedback.setProgressText(
                "Calculates average wind profile (within polygons)..."
            )
        # Load polygons
        loadFile(
            cursor=cursor,
        ...
        cursor.execute(
           ...
           """).format(
                DataUtil.createIndex(
                    tableName=allPointsTab,
                    fieldName=GEOM_FIELD,
                    isSpatial=True,
                ),
                DataUtil.createIndex(
                    tableName=polygonsTab, fieldName=GEOM_FIELD, isSpatial=True
                ),
                DataUtil.createIndex(
                    tableName=allPointsTab, fieldName=Z, isSpatial=False
                ),
                DataUtil.createIndex(
                    tableName=polygonsTab,
        ...
        for w in windList.index:
            ...
            for p in polygonsList:
                data2plot = df_selectedPolygons[
                    df_selectedPolygons[idPolygons.upper()] == p
                ].sort_values(Z)
                ax_poly[w].plot(
                    data2plot[w.upper()],
                    data2plot[Z],
                    label="Polygon {0}".format(p),
                )
    ...
    if lines_file and srid_lines and idLines:
        ...
        cursor.execute(
           ...
           """).format(
                ...
                DataUtil.createIndex(
                    tableName=linesTab, fieldName=GEOM_FIELD, isSpatial=True
                ),
                Z,
                WINDSPEED_X,
                WINDSPEED_Y,
        ...
        buildIndexAll = df_selectedPoints[
            (df_selectedPoints[WINDSPEED_X.upper()] == 0)
            & (df_selectedPoints[WINDSPEED_Y.upper()] == 0)
            & (df_selectedPoints[WINDSPEED_Z.upper()] == 0)
        ].index
        df_selectedPoints.loc[
            buildIndexAll,
            [WINDSPEED_X.upper(), WINDSPEED_Y.upper(), WINDSPEED_Z.upper()],
        ] = np.nan
        ...
        for line in sorted(set(df_selectedPoints[idLines.upper()])):
            ...
            df_plot = (
                df_selectedPoints[df_selectedPoints[idLines.upper()] == line]
                .groupby([Z.upper(), "DIST"])
                .mean()
            )
            ...
            if isStream:
                ...
                wind_d = np.array(
                    [
                        [
                            dic_all[line][zval].loc[d, "PROJECTED_HORIZ_WIND"]
                            for d in uniques_dist
                        ]
                        for zval in uniques_z
                    ]
                )
                wind_z = np.array(
                    [
                        [
                            dic_all[line][zval].loc[d, WINDSPEED_Z.upper()]
                            for d in uniques_dist
                        ]
                        for zval in uniques_z
                    ]
                ...
                ax[line].streamplot(
                    D, z, wind_d, wind_z, density=STREAM_DENSITY, color=color
                )
            else:
                ...
                wind_d = np.array(
                    [
                        [
                            df_plot.loc[zval, d]["PROJECTED_HORIZ_WIND"]
                            for d in uniques_dist
                        ]
                        for zval in uniques_z
                    ]
                ...
                wind_z = np.array(
                    [
                        [
                            df_plot.loc[zval, d][WINDSPEED_Z.upper()]
                            for d in uniques_dist
                        ]
                        for zval in uniques_z
                    ]
            ...
            buildIndexes = (
                df_plot[
                    np.isnan(df_plot[WINDSPEED_X.upper()])
                    & np.isnan(df_plot[WINDSPEED_Y.upper()])
                    & np.isnan(df_plot[WINDSPEED_Z.upper()])
                ]
            ...
            for i, bcell in enumerate(buildIndexes):
                ...
                rect[i] = Rectangle(
                    (rec_d0, rec_z0), rec_width, rec_height, color="grey"
                )
            ...
            if savePlot:
                fig[line].savefig(
                    os.path.join(
                        outputDirectory,
                        simulationName + "_line" + str(line) + ".png",
                    )
                )
            else:
...
def conditional_interpolate(df, cols, limit=2):
    ...
    m = (
        df[cols]
        .groupby([s, df[cols].isna().prod(axis=1) == 0])[cols[0]]
        .transform("size")
