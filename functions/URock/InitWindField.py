...
def createGrid(cursor, dicOfInputTables,  srid,
    ...
    gatherQuery = [safe("""SELECT {0} AS {0} FROM {1}""").format(GEOM_FIELD, 
                                                           dicOfInputTables[t])
...
def affectsPointToBuildZone(cursor, gridTable, dicOfBuildRockleZoneTable,
    ...
    for i, t in enumerate(dicOfBuildRockleZoneTable):
        ...
        queryKeepY =    safe("b.{1}, b.{0}, b.{2},").format(ID_POINT_X, Y_POINT, ID_POINT_Y)
        ...
        if t==STREET_CANYON_NAME:
            columnsToKeepQuery = safe("""b.{0}, {1} a.{2}, a.{3}, a.{4}, a.{5}, a.{6}
                                    """).format( ID_POINT, 
                                                queryKeepY,
                                                idZone[t],
            ...
            columnsToKeepQuery = safe("""b.{0}, a.{1}, a.{2}, b.{3}, a.{4}, a.{5}, a.{6}, a.{7}, 
                                   a.GEOM_CORNER_POINT,
                                   b.{8}
                                   """).format( ID_POINT, 
                                               idZone[t],
    ...
    cursor.execute(safe("""
       ...
       """).format(
           idx1=safe(DataUtil.createIndex(dicOfPrefixZoneLim[CAVITY_NAME], fieldName=DOWNWIND_FACADE_FIELD, isSpatial=False)),
           idx2=safe(DataUtil.createIndex(dicOfPrefixZoneLim[CAVITY_NAME], fieldName=ID_POINT_X, isSpatial=False)),
           idx3=safe(DataUtil.createIndex(dicOfPrefixZoneLim[WAKE_NAME], fieldName=DOWNWIND_FACADE_FIELD, isSpatial=False)),
           idx4=safe(DataUtil.createIndex(dicOfPrefixZoneLim[WAKE_NAME], fieldName=ID_POINT_X, isSpatial=False)),
           ...
           idx5=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[CAVITY_NAME]), fieldName=safe(DOWNWIND_FACADE_FIELD), isSpatial=False)),
           idx6=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[CAVITY_NAME]), fieldName=safe(ID_POINT_X), isSpatial=False)),
           idx7=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[STREET_CANYON_NAME]), fieldName=safe(DOWNWIND_FACADE_FIELD), isSpatial=False)),
           idx8=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[STREET_CANYON_NAME]), fieldName=safe(ID_POINT_X), isSpatial=False)),
           canyon_table=safe(dicOfPrefixZoneLim[STREET_CANYON_NAME]),
           idx9=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[CAVITY_NAME]), fieldName=safe(DOWNWIND_FACADE_FIELD), isSpatial=False)),
           idx10=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[CAVITY_NAME]), fieldName=safe(ID_POINT_X), isSpatial=False)),
           idx11=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[WAKE_NAME]), fieldName=safe(DOWNWIND_FACADE_FIELD), isSpatial=False)),
           idx12=safe(DataUtil.createIndex(safe(dicOfPrefixZoneLim[WAKE_NAME]), fieldName=safe(ID_POINT_X), isSpatial=False)),
    ...
    cursor.execute(safe("""DROP TABLE IF EXISTS {rooftop_table};
...
def calculates3dBuildWindFactor(cursor, dicOfBuildZoneGridPoint,
    ...
    cursor.execute(safe(""" SELECT MAX(MAX_HEIGHT) AS MAX_HEIGHT
                       FROM (SELECT {0})
...
def calculates3dVegWindFactor(cursor, dicOfVegZoneGridPoint, sketchHeight,
    ...
    unionAllQuery = [safe(" SELECT {0}, {1}, {2} FROM {3}").format(ID_POINT,
                                                             ID_POINT_Z,
                                                             VEGETATION_FACTOR,
                                                             dicOfTempoTables[t])
...
def manageSuperimposition(cursor,
    ...
    cursor.execute(safe("""
      ...
      """).format(
          idx1=DataUtil.createIndex(tableName=upstreamBackPrioritiesTempoTable, 
                                    fieldName=[ID_POINT, ID_POINT_Z],
                                    isSpatial=False),
          idx2=DataUtil.createIndex(tableName=tempoPrioritiesWeightedAll, 
                                    fieldName=[ID_POINT, ID_POINT_Z],
                                    isSpatial=False),
      ...
      CREATE TABLE {unique_table}
          ...
          FROM     {all_points_table} AS a RIGHT JOIN {unique_points_table} AS b
                   ON a.{id_3d} = b.{id_3d} AND 
                      a.{id_p} = b.{id_p}
      """).format(
          idx1=DataUtil.createIndex(tableName=tempoAllPointsTable, 
                                    fieldName=[ID_3D_POINT, ID_POINT],
                                    isSpatial=False),
          idx2=DataUtil.createIndex(tableName=tempoUniquePointsTable, 
                                    fieldName=[ID_3D_POINT, ID_POINT],
                                    isSpatial=False),
