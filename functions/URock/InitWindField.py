...
def createGrid(
    ...
    gatherQuery = [
        safe("""SELECT {0} AS {0} FROM {1}""").format(
            GEOM_FIELD, dicOfInputTables[t]
        )
...
def affectsPointToBuildZone(
    ...
    verticalLineTable = DataUtil.prefix(
        tableName="VERTICAL_LINES", prefix=prefix
    )
    ...
    cursor.execute(
       ...
       """).format(
            ...
            len_cav=safe(LENGTH_ZONE_FIELD + CAVITY_NAME[0]),
            cos_az=safe(COS_BLOCK_AZIMUTH),
            sin_az=safe(SIN_BLOCK_AZIMUTH),
            rel_pos=safe(DOWNSTREAM_X_RELATIVE_POSITION),
            wake_table=safe(dicOfPrefixZoneLim[WAKE_NAME]),
            cavity_table=safe(dicOfPrefixZoneLim[CAVITY_NAME]),
            downwind_f=safe(DOWNWIND_FACADE_FIELD),
            id_px=safe(ID_POINT_X),
            idx5=safe(
                DataUtil.createIndex(
                    safe(dicOfPrefixZoneLim[CAVITY_NAME]),
                    fieldName=safe(DOWNWIND_FACADE_FIELD),
                    isSpatial=False,
                )
            ),
            idx6=safe(
                DataUtil.createIndex(
                    safe(dicOfPrefixZoneLim[CAVITY_NAME]),
                    fieldName=safe(ID_POINT_X),
                    isSpatial=False,
                )
            ),
            idx7=safe(
                DataUtil.createIndex(
                    safe(dicOfPrefixZoneLim[STREET_CANYON_NAME]),
                    fieldName=safe(DOWNWIND_FACADE_FIELD),
                    isSpatial=False,
                )
            ),
            ...
            canyon_table=safe(dicOfPrefixZoneLim[STREET_CANYON_NAME]),
            idx9=safe(
                DataUtil.createIndex(
                    safe(dicOfPrefixZoneLim[CAVITY_NAME]),
                    fieldName=safe(DOWNWIND_FACADE_FIELD),
            ...
            idx11=safe(
                DataUtil.createIndex(
                    safe(dicOfPrefixZoneLim[WAKE_NAME]),
                    fieldName=safe(DOWNWIND_FACADE_FIELD),
                    isSpatial=False,
                )
            ),
            ...
            len_wake=safe(LENGTH_ZONE_FIELD + WAKE_NAME[0]),
        )
    )
    ...
    cursor.execute(
        safe("""DROP TABLE IF EXISTS {rooftop_table};
           CREATE TABLE {rooftop_table}
               AS SELECT {id_point},
                        ST_DISTANCE({geom}, GEOM_CORNER_POINT)/
                            COS(CASE WHEN   {angle}<PI()/2
                                THEN        {angle}-ST_AZIMUTH({geom}, GEOM_CORNER_POINT)
                                ELSE        ST_AZIMUTH(GEOM_CORNER_POINT, {geom})-{angle}
                                END
                                )/
                            {facade_len}*{corner_len} AS {var_height},
                        {id_zone},
                        {height},
                        {angle},
                        {wind_factor},
                        ...
                        {id_point_x}
                FROM {tempo_table}""").format(
            rooftop_table=dicOfOutputTables[ROOFTOP_CORN_NAME],
            id_point=ID_POINT,
            geom=GEOM_FIELD,
            angle=UPWIND_FACADE_ANGLE_FIELD,
            facade_len=ROOFTOP_CORNER_FACADE_LENGTH,
            corner_len=ROOFTOP_CORNER_LENGTH,
            var_height=ROOFTOP_CORNER_VAR_HEIGHT,
            id_zone=idZone[ROOFTOP_PERP_NAME],
            height=HEIGHT_FIELD,
            wind_factor=ROOFTOP_WIND_FACTOR,
            y_wall=Y_WALL,
            id_point_x=ID_POINT_X,
            tempo_table=dicOfTempoOutput[ROOFTOP_CORN_NAME],
        )
    )
...
def removeBuildZonePoints(
    cursor, dicOfInitBuildZoneGridPoint, prefix=PREFIX_NAME
):
    ...
    cursor.execute(
        safe("""
           ...
           """).format(
            ...
            DataUtil.createIndex(
                cavityFirstPoint, fieldName=ID_POINT, isSpatial=False
            ),
            DataUtil.createIndex(
                cavityFirstPoint,
                fieldName=ID_FIELD_STACKED_BLOCK,
    ...
    cursor.execute(
        safe(";").join(
           ...
           """).format(
                    dicOfBuildZoneGridPoint[t], dicOfInitBuildZoneGridPoint[t]
                )
...
def manageBackwardZones(
    ...
    dicOfTempoBackPoints = {
        t: DataUtil.postfix(
            tableName=DataUtil.prefix(tableName=t, prefix="TEMPO"),
            suffix="POINTS",
        )
...
def calculates3dBuildWindFactor(
    ...
    maxHeightQuery = {
        ...
        STREET_CANYON_NAME: "MAX({0}) AS MAX_HEIGHT".format(
            UPPER_VERTICAL_THRESHOLD
        ),
        ...
        CAVITY_BACKWARD_NAME: "MAX({0}) AS MAX_HEIGHT".format(
            UPPER_VERTICAL_THRESHOLD
        ),
        WAKE_BACKWARD_NAME: "MAX({0}) AS MAX_HEIGHT".format(
            UPPER_VERTICAL_THRESHOLD
        ),
    }
    cursor.execute(
        safe(""" SELECT MAX(MAX_HEIGHT) AS MAX_HEIGHT
                       FROM (SELECT {0})
                       """).format(
        ...
        if maxHeight > float(dz) / 2:
            ...
            cursor.execute(
                safe("""
                       ...
                       """).format(
                    zValueTable, "), (DEFAULT, ".join(listOfZ), ID_POINT_Z, Z
                )
            )
    ...
    calcQuery = {
        DISPLACEMENT_NAME: """
                 ...
                 a.{3}
                 """.format(
            ID_POINT_Z, C_DZ, Z, HEIGHT_FIELD, P_DZ, V, ID_POINT, U, W
        ),
        DISPLACEMENT_VORTEX_NAME: """
                 b.{0},
    ...
    whereQuery = {
        ...
        DISPLACEMENT_VORTEX_NAME: "b.{0} < a.{1}".format(
            Z, UPPER_VERTICAL_THRESHOLD
        ),
        ...
        CAVITY_BACKWARD_NAME: "b.{0} < a.{1}".format(
            Z, UPPER_VERTICAL_THRESHOLD
        ),
...
def calculates3dVegWindFactor(
    ...
    # Name of the output table
    vegetationWeightFactorTable = DataUtil.prefix(
        outputBaseName, prefix=prefix
    )
    ...
    calcQuery = {
        VEGETATION_OPEN_NAME: """ CASE WHEN (b.{2} - 3 * 0.05 * b.{2} <= {1} OR b.{2} <= {1}) AND a.{0} < b.{4}
                    THEN EXP(0)
        ...
        VEGETATION_BUILT_NAME: """ CASE WHEN (b.{2} <= {1} OR a.{0} <= {1}) AND a.{0} < b.{4}
                    THEN EXP(0)
    ...
    unionAllQuery = [
        safe(" SELECT {0}, {1}, {2} FROM {3}").format(
...
def manageSuperimposition(
    ...
    tempoPrioritiesWeightedAll = DataUtil.postfix(
        "TEMPO_PRIORITY_WEIGHTED_ALL"
    )
    ...
    tempoUpstreamAndDownstream = DataUtil.postfix(
        "TEMPO_UPSTREAM_AND_DOWNSTREAM"
    )
    ...
    cursor.execute(
      ...
      CREATE TABLE {result_table}
          ...
          UNION ALL
          SELECT  a.*
          FROM     {weighted_table} AS a LEFT JOIN {upstream_table} AS b
                   ON a.{id_p} = b.{id_p} AND a.{id_z} = b.{id_z}
      ...
      """).format(
            ...
            idx2=DataUtil.createIndex(
                tableName=tempoPrioritiesWeightedAll,
                fieldName=[ID_POINT, ID_POINT_Z],
                isSpatial=False,
            ),
            result_table=tempoPrioritiesWeightedAllPlusBack,
            upstream_table=upstreamBackPrioritiesTempoTable,
            weighted_table=tempoPrioritiesWeightedAll,
            id_p=ID_POINT,
            id_z=ID_POINT_Z,
        )
    )
...
def manageUpstreamSuperimposition(
    ...
    upstreamPrioritiesTempoTable = identifyUpstreamer(
        ...
        tablesToConsider=upstreamPriorityTables.reindex(
            upstreamPriorityTables.index.difference(
                pd.Index(upstreamWeightingTables)
            )
        ),
    ...
    cursor.execute(
        safe("""
          ...
          """).format(
            ...
            DataUtil.createIndex(
                tableName=tempoPrioritiesWeighted,
                fieldName=ID_POINT,
                isSpatial=False,
            ),
            DataUtil.createIndex(
...
def identifyUpstreamer(
    ...
    cursor.execute(
      ...
      """).format(
            ...
            idx2=DataUtil.createIndex(
                tableName=tempoUniquePointsTable,
                fieldName=[ID_3D_POINT, ID_POINT],
                isSpatial=False,
            ),
            unique_table=uniqueValuePerPointTable,
            all_points_table=tempoAllPointsTable,
            unique_points_table=tempoUniquePointsTable,
            id_3d=ID_3D_POINT,
            id_p=ID_POINT,
        )
    )
    ...
    **kwargs,
    ...
    if profileType == "power":
        ...
        verticalProfileWithin = pd.Series(
            [
                speedAtCanopyHeight * np.exp(A * (z / H - 1))
    ...
    verticalWindProfile = pd.DataFrame(
        {
            HORIZ_WIND_SPEED: verticalWindProfile.values,
...
def setInitialWindField(
    ...
    **kwargs,
    ...
    tempoBuildingHeightWindTable = DataUtil.postfix(
        "TEMPO_BUILDING_HEIGHT_WIND"
    )
    tempoZoneWindSpeedFactorTable = DataUtil.postfix(
        "TEMPO_ZONE_WIND_SPEED_FACTOR"
    )
    ...
    if len(buildingHeightList) > 0:
        df_buildingHeightList = pd.Series(
            pd.DataFrame(buildingHeightList)[0].values
        )
    ...
    if V_ref is None or z_ref is None:
        ...
        z_ref = verticalWindSpeedProfile.loc[
            verticalWindSpeedProfile.index[-1], Z
        ]
    ...
    cursor.execute(safe("""SELECT   MAX({0}) AS ID_POINT_X,
                               MAX({1}) AS ID_POINT_Y
                       FROM     {2}
...
def identifyBuildPoints(
    ...
    cursor.execute(
        safe("""
           ...
           """).format(
            ...
            DataUtil.createIndex(
                tableName=tempoLevelHeightPointTable,
                fieldName=Z,
                isSpatial=False,
            ),
        )
    ...
    df_wall_left = df_gridBuil.index.set_levels(
        df_gridBuil.index.levels[0] + 1, level=0
    )
    df_wall_right = df_gridBuil.index.set_levels(
        df_gridBuil.index.levels[0] - 1, level=0
    )
    df_wall_behind = df_gridBuil.index.set_levels(
        df_gridBuil.index.levels[1] + 1, level=1
    )
    df_wall_face = df_gridBuil.index.set_levels(
        df_gridBuil.index.levels[1] - 1, level=1
    )
    ...
    ind2remove = (
        ...
        .union(
            df_wall_left.intersection(df_wall_right).intersection(df_wall_face)
        )
        .union(
            df_wall_left.intersection(df_wall_behind).intersection(
                df_wall_face
            )
        )
        .union(
            df_wall_right.intersection(df_wall_behind).intersection(
                df_wall_face
            )
        )
