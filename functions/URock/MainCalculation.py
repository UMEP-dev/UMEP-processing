...
def main(
    ...
    outputDataRel["blocks"] = os.path.join(
        tmp_dir_unique, f"blocks.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["stacked_blocks"] = os.path.join(
        tmp_dir_unique, f"stackedBlocks.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["vegetation"] = os.path.join(
        tmp_dir_unique, f"vegetation.{OUTPUT_VECTOR_EXTENSION}"
    )

    # Rotated geometries
    outputDataRel["rotated_stacked_blocks"] = os.path.join(
        tmp_dir_unique, f"rotated_stacked_blocks.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["rotated_vegetation"] = os.path.join(
        tmp_dir_unique, f"vegetationRotated.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["upwind_facades"] = os.path.join(
        tmp_dir_unique, f"upwind_facades.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["downwind_facades"] = os.path.join(
        tmp_dir_unique, f"downwind_facades.{OUTPUT_VECTOR_EXTENSION}"
    )

    # Created zones
    outputDataRel["displacement"] = os.path.join(
        tmp_dir_unique, f"displacementZones.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["displacement_vortex"] = os.path.join(
        tmp_dir_unique, f"displacementVortexZones.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["cavity"] = os.path.join(
        tmp_dir_unique, f"cavity.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["wake"] = os.path.join(
        tmp_dir_unique, f"wake.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["street_canyon"] = os.path.join(
        tmp_dir_unique, f"streetCanyon.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["rooftop_perpendicular"] = os.path.join(
        tmp_dir_unique, f"rooftopPerp.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["rooftop_corner"] = os.path.join(
        tmp_dir_unique, f"rooftopCorner.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["vegetation_built"] = os.path.join(
        tmp_dir_unique, f"vegetationBuilt.{OUTPUT_VECTOR_EXTENSION}"
    )
    outputDataRel["vegetation_open"] = os.path.join(
        tmp_dir_unique, f"vegetationOpen.{OUTPUT_VECTOR_EXTENSION}"
    )
    ...
    cursor, conn, localH2InstanceDir = H2gisConnection.startH2gisInstance(
        dbDirectory=dBDir, dbInstanceDir=tmp_dir_unique, suffix=db_suffix
    )
    ...
    if debug or saveRockleZones:
        for t in dicOfBuildZone3DWindFactor:
            ...
            saveData.saveTable(
                ...
                filedir=outputDataAbs["point3D_BuildZone"]
                + t
    ...
    if not debug:
        H2gisConnection.closeAndRemoveH2gisInstance(
            localH2InstanceDir=localH2InstanceDir, conn=conn, cur=cursor
        )
