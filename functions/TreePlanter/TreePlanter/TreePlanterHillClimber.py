    ...
    for counter in range(r_iters):
        ...
        if counter == iters_progress[progress]:
            feedback.setProgressText(
                str(percentage_progress[progress] * 100)
        ...
        while r_count < 1:
            ...
            if sa == 0:
                ...
                (
                    ...
                    tp_c,
                    break_loop,
                ) = StartingPositions.genetic_start(
                    tree_pos_x,
                    tree_pos_y,
                    tree_pos_c,
                    positions,
            ...
            if tp_c == 100:
                feedback.setProgressText(
                    "Possibly too many trees to fit in planting area. Try a lower number."
                )
                break
            ...
            if (counter == 0) | (sa == 0):
                ...
                for i2 in range(tree_pos.__len__()):
                    tree_pos_x[i2] = positions.pos[
                        positions.pos[:, 0] == tree_pos[i2], 1
                    ]
                    tree_pos_y[i2] = positions.pos[
        ...
        while np.sum(tp_nc[:, 0]) < trees:
            ...
            t1, nc, y_out, x_out = HillClimberAlgorithm.topt(
                tree_pos_y,
                tree_pos_x,
                treerasters,
                treeinput,
        ...
        if sa == 1:
            if i_tmrt[counter] > i_tmrt_max:
                ...
                for idx in range(trees):
                    d_tmrt_temp[idx] = treerasters.d_tmrt[
                        tree_pos_y[idx], tree_pos_x[idx]
                    ]
