...
def treenudge(
    ...
    if (
        ...
        (
            ...
            nc_y, nc_x, treerasters, treeinput
        )  # Shadows of trees

        if (
            compare == 1
        ):  # If there are less regional groups than trees standing still, some must be side by side
            ...
            if (nc_i_r[0].__len__() > 1) & (nc_sum == nc_i_r[0].__len__()):
                ...
                for iy in range(y_dir.shape[0]):
                    ...
                    for iy1 in range(y_dir_temp.shape[0]):
                        if not (
                            np.any(
                                positions.pos[
                                    (positions.pos[:, 1] == x_dir_temp[iy1])
                                    & (positions.pos[:, 2] == y_dir_temp[iy1]),
                                    0,
                                ]
                            )
                        ):
                ...
                if np.any(
                    ...
                    for iy in range(
                        ...
                        nc_x_c[nc_i_r] = (
                            np.squeeze(x_out[nc_i_r]) + x_dir[iy]
                        )  # New x-position

                        tsh_bool_nc_t, tsh_bool_nc_t_large, comp_ = tsh_gen_ts(
                            nc_y_c, nc_x_c, treerasters, treeinput
                        )  # Shadows and rg for new position
                        ...
                        for j in range(tmrt_1d.shape[0]):
                            ...
                            tmrt_nc[iy, 1] += np.sum(
                                tsh_bool_nc_t[:, :, j] * treeinput.tmrt_ts[:, :, j]
                            )  # Sunlit
                        tmrt_nc[iy, 2] = (
                            tmrt_nc[iy, 1] - tmrt_nc[iy, 0]
                        )  # New Tmrt (sunlit - tree shade)

                    if np.around(np.max(tmrt_nc[:, 2]), decimals=1) > np.around(
                        ...
                        nc_y_out = (
                            np.squeeze(y_out[nc_i_r]) + y_dir[nc_max_r]
                        )  # New y-positions
                        nc_x_out = (
                            np.squeeze(x_out[nc_i_r]) + x_dir[nc_max_r]
                        )  # New x-positions
...
def tree_adjust(i_y, i_x, i_tmrt, counter, trees, treerasters, treeinput):
    ...
    while a_counter < trees:
        ...
        for ix in range(trees):
            tmrt_temp[ix] = treerasters.d_tmrt[
        ...
        )  # Shadows for high trees
        tsh_rg_min, tsh_bool_min, comp__ = tsh_gen_ts(
            y_low, x_low, treerasters, treeinput
        )  # Shadows for low tree

        d_tmrt_pad = np.pad(
            treerasters.d_tmrt,
            pad_width=(
                (treerasters.tpy[0], treerasters.tpy[0]),
                (treerasters.tpx[0], treerasters.tpx[0]),
            ),
            mode="constant",
            constant_values=0,
        )  # Padded sun vs shade tmrt rasters
