    ...
    for tree in range(trees):
        ...
        for i in range(res_y.__len__()):
            ...
            for j in range(tmrt_1d.__len__()):
                ...
                sum_tmrt_tsh[res_y[i], res_x[i]] += np.sum(
                    ts_temp2
                    * treeinput.buildings
                    * shadows_copy[:, :, j]
                    * tmrt_1d[j, 0]
        ...
        for j in range(tmrt_1d.__len__()):
            ...
            temp_shadow[yslice2, xslice2] = treerasters.treeshade_bool[
                yslice1, xslice1, j
            ]
        ...
        yslice1, xslice1, yslice2, xslice2 = tree_slice(
            yt1, yt2, xt1, xt2, treeinput, treerasters
        )
        ...
        if (np.max(recalc_positions) == 0) & (tree != trees - 1):
            ...
            feedback.setProgressText(
                "Not enough space for all trees! Found locations for "
                + str(int(best_y.shape[0]))
                + " out of "
                + str(int(trees))
                + " trees."
            )
