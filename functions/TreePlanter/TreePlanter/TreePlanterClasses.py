...
class Inputdata:
    ...
    def __init__(
        ...
        for iy in r_range:
            ...
            self.tmrt_ts[:, :, c] = (
                np.around(dataSet2.ReadAsArray().astype(float), decimals=1)
                * self.shadow[:, :, c]
            )
        ...
        rasterize_options = gdal.RasterizeOptions(
            options=[
                ...
                str(maxy),
                "-tr",
                str(np.abs(self.gt[1])),
                str(np.abs(self.gt[5])),
            ]
        )

        gdal.Rasterize(
            infolder + "/selected_area.tif",
            inputPolygonlayer,
...
class Regional_groups:
    ...
    def __init__(self, range_, shadow_, shadow_ts, tmrt):
        ...
        sh_vec_t, sh_idx, sh_inv = np.unique(
            sh_vec_t, return_index=True, return_inverse=True, axis=0
        )
...
class ClippedInputdata:
    ...
    def __init__(self, treeinput, treerasters):
        ...
        self.clip_rows = np.int_(self.clip_rows)
        self.clip_cols = np.int_(self.clip_cols)

        # Clip input rasters
        self.buildings = treeinput.buildings[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        self.selected_area = treeinput.selected_area[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        # self.dem = treeinput.dem[self.clip_rows[0]:self.clip_rows[1], self.clip_cols[0]:self.clip_cols[1]]
        self.dsm = treeinput.dsm[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        self.cdsm = treeinput.cdsm[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        self.cdsm_b = treeinput.cdsm_b[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        self.shadow = treeinput.shadow[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        self.tmrt_ts = treeinput.tmrt_ts[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
        self.tmrt_s = treeinput.tmrt_s[
            self.clip_rows[0] : self.clip_rows[1],
            self.clip_cols[0] : self.clip_cols[1],
        ]
