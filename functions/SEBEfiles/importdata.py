...
def importdata(*args):
    ...
    if nargin > 2:
        ...
        if not isinstance(headerRows, int) or headerRows < 0:
            raise TypeError(
                "importdata: Number of header rows needs to be an integer number >= 0."
            )
...
def importdata_ascii(fileName, delimiter, headerRows):
    ...
    if not delimiter:
        raise ValueError(
            "importdata: Guessing delimiter is not implemented yet, you have to specify it."
        )
    ...
    fileContentRows = fileContentRows[:headerRows] + [
        line for line in fileContentRows[headerRows:] if not len(line) < 1
    ]
