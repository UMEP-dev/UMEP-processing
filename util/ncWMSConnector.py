...
class NCWMS_Connector(object):
    ...
    def check_bbox(
        self, lower_left_lat, lower_left_lon, upper_right_lat, upper_right_lon
    ):
        """
        ...
        if lower_left_lat > upper_right_lat:
            raise InvalidRelativeBbox(
                "Lower left latitude of bounding box cannot be greater than upper right latitude"
            )

        if lower_left_lon > upper_right_lon:
            raise InvalidRelativeBbox(
                "Lower left longitude of bounding box cannot be greater than upper right longitude"
            )
    ...
    def check_times(self, start_date, end_date):
        ...
        if type(start_date) is not dt:
            raise TypeError(
                "Start time is of type %s, but must be datetime.datetime"
                % (type(start_date),)
            )
        if type(end_date) is not dt:
            raise TypeError(
                "End time is of type %s, but must be datetime.datetime"
                % (type(start_date),)
            )

        if start_date > end_date:
            raise InvalidTimeWindow(
                "Requested end date (%s) must be greater than or equal to start date (%s)"
                % (
        ...
        if (start_date < self.start_date) or (start_date > self.end_date):
            raise InvalidTimeWindow(
                "Requested start date (%s) must be between %s and %s"
                % (
                    end_date.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        if (end_date > self.end_date) or (end_date < self.start_date):
            raise InvalidTimeWindow(
                "Requested end date (%s) must be between %s and %s"
                % (
                    end_date.strftime("%Y-%m-%d %H:%M:%S"),
                    self.start_date.strftime("%Y-%m-%d %H:%M:%S"),
                    self.end_date.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def check_vars(self, var_list):
        ...
        for v in var_list:
            if v not in self.vars:
                raise ValueError(
                    "Variable %s is not one of those available in data" % (v,)
                )
    ...
    def average_data(self, period, method):
        ...
        if period is None:
            ...
            new_time_bins = self.resample_by_method(
                pd.Series(index=time_bins, data=0).resample(
                    "%ds" % (period,), label="right"
                ),
                method,
            ).index
        ...
        for v_name, varin in combined_data.variables.items():
            ...
            if v_name in self.vars:
                ...
                for i in range(0, combined_data.variables[v_name][:].shape[1]):
                    for j in range(
                        ...
                        if period is None:
                            ...
                            rs = self.resample_by_method(
                                p.resample("%ds" % (period,), label="right"),
                                method,
                            )
                        if (i == 0) and (j == 0):
                ...
                if v_name == "time":
                    ...
                    timestamps = nc4.date2num(
                        list(new_time_bins),
                        varin.units.replace("seconds", "hours"),
                    )  # Replace seconds with hours to save data
                    outVar[:] = timestamps
    ...
    def retrieve(self, start_period, end_period):
        ...
        resp = requests.get(
            baseURL,
            params=parms,
            auth=HTTPDigestAuth("umep-user", "pUEmw5BbVdzfu3dz"),
            stream=True,
            timeout=15,
        )
        if resp.status_code != 200:
            raise Exception(
                "Error connecting to server. Got HTTP response code %d"
                % (resp.status_code,)
            )
