...
def Solweig_2015a_metdata_noload(inputdata, location, UTC):
    ...
    for i, row in enumerate(met[:, 0]):
        ...
        if (i == 0) or (np.mod(dectime[i], np.floor(dectime[i])) == 0):
            ...
            sunmaximum = -90.0
            sunmax["zenith"] = 90.0
            while sunmaximum <= 90.0 - np.array(sunmax["zenith"]).item():
                sunmaximum = 90.0 - np.array(sunmax["zenith"]).item()
        ...
        sun_zenith = np.array(sun["zenith"]).item()
        sun_azimuth = np.array(sun["azimuth"]).item()

        if (sun_zenith > 89.0) & (sun_zenith <= 90.0):
            sun_zenith = 89.0

        altitude[0, i] = 90.0 - sun_zenith
        zen[0, i] = sun_zenith * (np.pi / 180.0)
