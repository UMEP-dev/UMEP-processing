...
def read_solweig_config(config_file_path):
    if not os.path.exists(config_file_path):
        raise ValueError(
            "The config file '{}' could not be found.".format(config_file_path)
        )
...
def write_solweig_config(configDict, refdir):
    ...
    f.write(
        "# estimate diffuse and global shortwve radiation from global radiation (1)\n"
    )
    ...
    f.write(
        "# use dem and dsm to identify building pixels (0) else, land cover will be used (0)\n"
    )
