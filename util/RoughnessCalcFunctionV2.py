...
def RoughnessCalcMany(Roughnessmethod, zH, fai, pai, zMax, zSdev):
    ...
    z_0_output = np.zeros((fai.shape[0], 1)) - 999.0
    ...
    for i in range(0, fai.shape[0]):
        if Roughnessmethod == "RT":
            ...
            if pai[i] >= 0.19:
                ZdMho_U = (
                    ((19.2 * pai[i]) - 1 + (np.exp(-19.2 * pai[i])))
                    / ((19.2 * pai[i]) * (1 - (np.exp(-19.2 * pai[i]))))
                ) * zH[i]
            elif pai[i] < 0.19:
                ZdMho_U = (
            ...
            ZdMho_UCor = zH[i] * (
                (ZdMho_U / zH[i])
                + ((0.2375 * np.log(pai[i]) + 1.1738) * (zSdev[i] / zH[i]))
            )
            ZoMho_UCor = zH[i] * (
                (ZoMho_U / zH[i])
                + (
                    np.exp((0.8867 * fai[i]) - 1)
                    * ((zSdev[i] / zH[i]) ** np.exp(2.3271 * fai[i]))
                )
            )
