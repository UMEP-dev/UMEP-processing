...
class ProcessingTreeGeneratorAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_BUILD, self.tr("Building grid"), "", optional=True
            )
        )
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_DSM,
                self.tr(
                    "Raster DSM (3D objects and ground). Use if building grid is unavailable."
                ),
                "",
                optional=True,
            )
        ...
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_DEM,
                self.tr(
                    "Raster DEM (only ground). Use if building grid is unavailable."
                ),
                "",
                optional=True,
            )
        ...
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_CDSM,
                self.tr("Merge with existing vegetation Canopy (C)DSM"),
                "",
                optional=True,
            )
        )
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_TDSM,
                self.tr("Merge with existing vegetation Trunk Zone (T)DSM"),
                "",
                optional=True,
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        outputTDSM = self.parameterAsOutputLayer(
            parameters, self.TDSM_GRID_OUT, context
        )
        ...
        if cdsm:  # vegetation cdsm
            ...
            if tdsm is None:
                ...
                feedback.setProgressText(
                    "No TDSM raster layer is selected. Creating new TDSM raster layer."
                )
        ...
        if build:
            if cdsm:
                if not (
                    ...
                    raise QgsProcessingException(
                        "Error! Check the coordinate systems of your input data. Have to match!"
                    )
                    return
            else:
    ...
    def shortHelpString(self):
        return self.tr(
            "Information 3d vegetation is not a common spatial information available. "
