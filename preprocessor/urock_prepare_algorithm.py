...
class URockPrepareAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                ...
                self.tr(
                    "Buildings raster DSM (3D objects + ground or only 3D objects) [m asl or m agl]"
                ),
                None,
                optional=True,
            )
        ...
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                ...
                self.tr(
                    "DEM (ground - only if building DSM is 3D objects + ground) [m asl]"
                ),
                None,
                optional=True,
            )
        ...
        self.addParameter(
            QgsProcessingParameterString(
                ...
                self.tr(
                    "Tree height / tree crown radius ratio used if either height or radius value is missing"
                ),
                defaultValue=0.75,
                optional=True,
            )
        ...
        self.addParameter(
            QgsProcessingParameterVectorDestination(
                ...
                defaultValue=os.path.join(
                    TEMPO_DIRECTORY, f"build_vector{OUTPUT_VECTOR_EXTENSION}"
                ),
            )
        )
        self.addParameter(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        inputBuildinglayer = self.parameterAsVectorLayer(
            parameters, self.INPUT_BUILD_FOOTPRINT, context
        )
        inputVeglayer = self.parameterAsVectorLayer(
            parameters, self.INPUT_VEG_POINTS, context
        )

        # Get vegetation height attribute
        heightVegField = self.parameterAsString(
            parameters, self.HEIGHT_VEG_FIELD, context
        )
        radiusVegField = self.parameterAsString(
            parameters, self.RADIUS_VEG_FIELD, context
        )

        # Get some other parameters related to vegetation
        vegetationAspect = self.parameterAsDouble(
            parameters, self.VEGETATION_ASPECT, context
        )

        # Get building raster layers
        build_dsm = self.parameterAsRasterLayer(
            parameters, self.INPUT_BUILD_DSM, context
        )
        ...
        outputBuildFilepath = self.parameterAsOutputLayer(
            parameters, self.OUTPUT_BUILDING_FILE, context
        )
        buildingHeightField = self.parameterAsString(
            parameters, self.OUTPUT_BUILD_HEIGHT_FIELD, context
        )
        outputVegFilepath = self.parameterAsOutputLayer(
            parameters, self.OUTPUT_VEGETATION_FILE, context
        )
        vegetHeightField = self.parameterAsString(
        ...
        if inputBuildinglayer and build_dsm:
            ...
            if srid_vbuild != srid_dsm_build:
                build_dsm = processing.run(
                    "gdal:warpreproject",
                    {
                        "INPUT": build_dsm,
                        "SOURCE_CRS": QgsCoordinateReferenceSystem(
                            "EPSG:{0}".format(srid_dsm_build)
                        ),
                        "TARGET_CRS": QgsCoordinateReferenceSystem(
                            "EPSG:{0}".format(srid_vbuild)
                        ),
                        "RESAMPLING": 0,
                        "NODATA": None,
            ...
            tempoBuildinglayer = processing.run(
            ...
            outputBuildFilepath = processing.run(
                "native:renametablefield",
                {
                    "INPUT": tempoBuildinglayer2,
                    "FIELD": "height_median",
        ...
        # In case user provide a DSM and a DEM but no building footprint
            ...
            feedback.pushWarning(
                "You have provided a DSM but no building footprint. Please note that if you want to create a building layer for URock, you need to provide a building footprint in this preprocessing"
            )
        ...
        if veg_dsm and inputVeglayer:
            ...
            if veg_dsm:
                ...
                if not srid_vveg:
                    feedback.pushWarning(
                        "Note that your vegetation layer has no SRID, thus the output has also no SRID"
                    )

                # Round raster values in order to have less vegetation polygons
                veg_dsm = processing.run(
                ...
                veg_dsm_rounded = processing.run(
                    "native:roundrastervalues",
                    {
                        "INPUT": veg_dsm,
                        "BAND": 1,
                ...
                tempoVegFilepath = processing.run(
                    "native:dissolve",
                    {
                        "INPUT": veg_vect,
                        "FIELD": ["VALUE"],
                ...
                tempoVegFilepath2 = processing.run(
                    "native:extractbyattribute",
                    {
                        "INPUT": tempoVegFilepath,
                        "FIELD": "VALUE",
                        "OPERATOR": 2,
                        "VALUE": "0",
                        "OUTPUT": "TEMPORARY_OUTPUT",
                ...
                tempoVegFilepath3 = processing.run(
                    "native:assignprojection",
                    {
                        "INPUT": tempoVegFilepath2,
                        "CRS": QgsCoordinateReferenceSystem(
                            "EPSG:{0}".format(srid_vveg)
                        ),
                        "OUTPUT": "TEMPORARY_OUTPUT",
                    },
                )["OUTPUT"]
                ...
                outputVegFilepath = processing.run(
                    "native:renametablefield",
                    {
                        "INPUT": tempoVegFilepath3,
                        "FIELD": "VALUE",
                        "NEW_NAME": vegetHeightField,
                        "OUTPUT": outputVegFilepath,
                    },
                )["OUTPUT"]
            ...
            # Create vegetation patches from vegetation points
                ...
                if not radiusVegField and not heightVegField:
                    raise QgsProcessingException(
                        "At least tree crown radius or tree height attribute should be provided"
                    )
                else:
                    ...
                    tempoVegetationlayer = processing.run(
                        "native:buffer",
                        {
                            "INPUT": inputVeglayer,
        ...
        if veg_dsm:
            return {
                self.OUTPUT_BUILDING_FILE: outputBuildFilepath,
    ...
    def shortHelpString(self):
        return self.tr(
            "The urock_prepare is used to create building "
