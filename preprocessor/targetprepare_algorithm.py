...
class ProcessingTARGETPrepareAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterFile(
                self.INPUT_LC,
                self.tr("Land cover fraction file (.txt)"),
                extension="txt",
            )
        )
        self.addParameter(
        ...
        self.addParameter(
            QgsProcessingParameterString(self.SITE_NAME, self.tr("Site name"))
        )
        self.addParameter(
            QgsProcessingParameterFolderDestination(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not grid_unit in possible_units_metre:
            raise QgsProcessingException(
                "Error! Vector polygon projection is not in meters. Please reproject."
            )
                    ...
                    if feat_id == int(split[0]):
                        ...
                        if pai == 1:  # response to #109
                            feedback.pushWarning(
                                "Building fraction = 1 in grid: "
                                + str(feat_id)
                                + ". This is unlikely. Check your DSM and DEM. Land cover roof fraction will be used."
                            )
                            pai = roof
                            if pai == 1:
                                feedback.pushWarning(
                                    "Building fraction form land cover = 1 in grid: "
                                    + str(feat_id)
                                    + ". This is unlikely. Check your data."
                                )
                                pai = 0.99
