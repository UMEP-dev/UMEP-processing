...
class ProcessingUWGProcessorAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.OUTPUT_FORMAT,
                self.tr(
                    "Save output in UMEP specific format (required for the UWG Analyzer)"
                ),
            )
        )
            ...
            if excludeRural:
                if uwgDict["bldDensity"] < 0.005:
                    feedback.setProgressText(
                        "Grid: "
                        + str(attr)
                        + " not calculated. Less than 0.005 in building fraction."
                    )

                    if umepformat:
                        epwdata_uwg = np.genfromtxt(
                            epw_path,
                            skip_header=8,
                            delimiter=",",
                            filling_values=99999,
    ...
    def epw2UMEP(self, met_old):
        ...
        met_new[np.where(met_new[:, 2] == 0), 1] = (
            met_new[np.where(met_new[:, 2] == 0), 1] + 1
        )
