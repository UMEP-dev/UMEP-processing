...
class ProcessingTargetProcessorAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterString(self.RUN_NAME, self.tr("Run name"))
        )
        self.addParameter(
            QgsProcessingParameterDateTime(
                self.START_DATE,
                self.tr(
                    "Start date of simulation (should be a minimum of 24 hours prior to date for period of interest)"
                ),
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.MOD_LDOWN,
                self.tr(
                    "Estimate incoming longwave radiation from air temperture and relative humidity"
                ),
                defaultValue=False,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.OUTPUT_CSV, self.tr("Save output as .csv text files")
            )
        )
        self.addParameter(
            QgsProcessingParameterBoolean(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        stopDateInterest = self.parameterAsString(
            parameters, self.STOP_DATE_INTEREST, context
        )
        ...
        metfile.to_csv(
            inputDir + "/input/MET/" + runName + "_metdata.txt",
            sep=",",
            header="datetime,Ta,RH,WS,P,Kd,Ld",
        )
        ...
        cfM["date1"] = (
            datetime.datetime.strptime(startDateInterest, "%Y-%m-%d").strftime(
                "%Y,%m,%d"
            )
            + ",0"
        )
        cfM["date2"] = (
            datetime.datetime.strptime(stopDateInterest, "%Y-%m-%d").strftime(
                "%Y,%m,%d"
            )
            + ",0"
        )
        ...
        if date1a >= date1:
            raise QgsProcessingException(
                "Error: Start date should be at least 24h before Start date for period of interest"
            )
        if date1 >= date2:
            raise QgsProcessingException(
                "Error: Start date of Interest should be later than End date for period of interest"
            )
        ...
        if startmetfile <= date1a <= endmetfile:
            ...
            raise QgsProcessingException(
                "Error: Start date selected is not within dates available in the meteorological focring file"
            )
        if startmetfile <= date2 <= endmetfile:
            test = 4
        else:
            raise QgsProcessingException(
                "Error: End date selected is not within dates available in the meteorological focring file"
            )
        ...
        if umepformat:
            ...
            numformat = (
                "%d %d %d %d %.2f %.2f %.2f %.2f %.2f %.5f %.2f %.2f %.2f %.2f %.2f %.2f %.2f "
            ...
            np.savetxt(
                inputDir + "/output/" + runName + "_metdata_UMEP.txt",
                dfin,
                fmt=numformat,
                header=header,
                comments="",
