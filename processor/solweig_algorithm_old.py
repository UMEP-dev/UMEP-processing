...
class ProcessingSOLWEIGAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        miny = (
            geotransform[3]
            + widthx * geotransform[4]
            + heightx * geotransform[5]
        )
        ...
        if poilyr is not None:  # usePOI:
            ...
            poi_field = self.parameterAsFields(
            ...
            for f in vlayer.getFeatures():  # looping through each POI
                ...
                if miny >= 0:
                    poisxy[ind, 2] = np.round(
                        (miny + rows * (1.0 / scale) - y) * scale
                    )
                else:
                    poisxy[ind, 2] = np.round(
                        (miny + rows * (1.0 / scale) - y) * scale
            ...
            sensorheight = self.parameterAsDouble(
            ...
            feedback.setProgressText(
                "Point of interest (POI) vector data successfully loaded"
            )
        ...
        if landcover == 1.0:
            ...
            # Bogren et al. 2000, explained in Lindberg et al., 2008 and
            # Lindberg, Onomura & Grimmond, 2016)
            [
                TgK,
                Tstart,
                alb_grid,
                ...
                TmaxLST,
                TmaxLST_wall,
            ] = Tgmaps_v1(lcgrid.copy(), solweig_parameters)

        else:
            TgK = (
                Knight
                + solweig_parameters["Ts_deg"]["Value"]["Cobble_stone_2014a"]
            )
            Tstart = (
                Knight
                - solweig_parameters["Tstart"]["Value"]["Cobble_stone_2014a"]
            )
            TmaxLST = solweig_parameters["TmaxLST"]["Value"][
                "Cobble_stone_2014a"
            ]
            alb_grid = (
                Knight
        ...
        if folderWallScheme:
            ...
            wall_type = str(
                100
                + int(
                    self.parameterAsString(parameters, self.WALL_TYPE, context)
                )
            )

            # Calculate wall height for wall scheme, i.e. include corners
            # (thicker walls)
            walls_scheme = wa.findwalls_sp(
                dsm, 2, np.array([[1, 1, 1], [1, 0, 1], [1, 1, 1]])
            )
            # Calculate wall aspect for wall scheme, i.e. include corners
            # (thicker walls)
            ...
            wallScheme_feedback = (
                "Running with wall parameterization scheme. Walls divided into "
                + str(int(voxelTable.shape[0]))
                + " unique voxels."
            )
            ...
            if woilyr is not None:
                (
                    ...
                    dsm_y_rotation,
                    dsm_y_size,
                ) = gdal_dsm.GetGeoTransform()

                woi_field = self.parameterAsStrings(
                    parameters, self.WOI_FIELD, context
                )
                woisxy, woiname = wallOfInterest(
                    woilyr,
                    woi_field,
                    minx,
                    miny,
            ...
            # DataSet where wall temperatures/radiation is stored and
            # eventually saved as a NetCDf
            if wallNetCDF:
                met_for_xarray = (
        ...
        if anisotropic_sky:
            if not poisxy is None:
                patch_characteristics = np.zeros(
                    (shmat.shape[2], poisxy.shape[0])
                )
                for idx in range(poisxy.shape[0]):
                    for idy in range(shmat.shape[2]):
                        # Calculations for patches on sky, shmat = 1 = sky is
                        # visible
                        temp_sky = (shmat[:, :, idy] == 1) & (
                            vegshmat[:, :, idy] == 1
                        )
                        # Calculations for patches that are vegetation,
                        # vegshmat = 0 = shade from vegetation
                        temp_vegsh = (vegshmat[:, :, idy] == 0) | (
                            vbshvegshmat[:, :, idy] == 0
                        )
                        # Calculations for patches that are buildings, shmat =
                        # 0 = shade from buildings
                        temp_vbsh = (1 - shmat[:, :, idy]) * vbshvegshmat[
                            :, :, idy
                        ]
                        temp_sh = temp_vbsh == 1
                        if wallScheme:
                            temp_sh_w = temp_sh * voxelMaps[:, :, idy]
                            temp_sh_roof = temp_sh * (
                                voxelMaps[:, :, idy] == 0
                            )
                        else:
                            temp_sh_w = 0
                        ...
                        # Vegetation patch
                            ...
                            int(poisxy[idx, 2]), int(poisxy[idx, 1])
                        ]:
        ...
        for i in np.arange(0, Ta.__len__()):
            ...
            # Tgmap1, Tgmap1E, Tgmap1S, Tgmap1W, Tgmap1N, CI, TgOut1, diffsh,
            # ani)
            ...
            # timeaddW, timeaddN, timestepdec, Tgmap1, Tgmap1E, Tgmap1S,
            # Tgmap1W, Tgmap1N, CI, TgOut1, diffsh, ani)
            ...
            if folderWallScheme:
                ...
                if not woisxy is None:
                    for k in range(0, woisxy.shape[0]):
                        ...
                        data_out = (
                            outputDir + "/WOI_" + str(woiname[k]) + ".txt"
                        )
                        if i == 0:
                            # Output file header
            ...
            if (anisotropic_sky == 1) & (i == 0) & (not poisxy is None):
                for k in range(poisxy.shape[0]):
                    ...
                    skyviewimage_out = (
                        outputDir + "/POI_" + str(poiname[k]) + ".png"
                    )
                    PolarBarPlot(
                        Lsky_patch_characteristics,
                        altitude[0][i],
        ...
        if outputTreeplanter:
            ...
            np.savetxt(
                outputDir + "/treeplantersettings.txt",
                settingsData,
                fmt=settingsFmt,
