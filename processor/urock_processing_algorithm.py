    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if inputBuildinglayer and inputBuildinglayer.featureCount() > 0:
            ...
            if build_file.split(".")[-1].lower() == "gpkg":
                new_build_file = os.path.join(
                    TEMPO_DIRECTORY,
                    BUILDING_FILENAME + OUTPUT_VECTOR_EXTENSION,
                )
                processing.run(
                    "native:savefeatures",
                    {
                        "INPUT": build_file,
                        "OUTPUT": new_build_file,
                        "LAYER_NAME": "",
                        "DATASOURCE_OPTIONS": "",
                        "LAYER_OPTIONS": "",
                    },
        ...
        if inputVegetationlayer and inputVegetationlayer.featureCount() > 0:
            ...
            if veg_file.split(".")[-1].lower() == "gpkg":
                new_veg_file = os.path.join(
                    TEMPO_DIRECTORY,
                    VEGETATION_FILENAME + OUTPUT_VECTOR_EXTENSION,
                )
                processing.run(
                    "native:savefeatures",
                    {
                ...
                if saveVector:
                    ...
                    if not loadedVector.isValid():
                        ...
                        loadedVector.loadNamedStyle(
                            os.path.join(
                                resourceDir, "Resources", VECTOR_STYLE_FILENAME
                            ),
                            True,
                        )
